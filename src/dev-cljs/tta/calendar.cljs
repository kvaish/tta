...
(defonce state (r/atom
                {:start {:day 12, :month 3, :year 2018}
                 :end {:day 24, :month 3, :year 2018}}))

(defn datepickers-test []
  [:div
   [date-picker {:date (:start @state)
                 :valid-range {:start {:day 10, :month 3, :year 2018}
                              :end {:day 20, :month 3, :year 2018}}
                 :on-select #(swap! state assoc :start %)}]
   [date-range-picker {:start (:start @state)
                       :end (:end @state)
                       :valid-range {:start {:day 5, :month 3, :year 2018}
                                     :end {:day 25, :month 4, :year 2018}}
