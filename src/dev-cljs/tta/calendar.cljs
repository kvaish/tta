...
(def s-week-header {
    ...
    :vertical-align "top"
    :text-align "center"
    :color (htstyle/colors :slate-grey)
})
...
(def s-date {
    ...
    :vertical-align "top"
    :text-align "center"
    :color (htstyle/colors :royal-blue)
})
...
(defn- s-full-toggle-button [is-selected?] (merge {
    :background-color (if is-selected? (htstyle/colors :sky-blue) (htstyle/colors :white))
    :color (if is-selected? (htstyle/colors :white) (htstyle/colors :sky-blue))
    :text-align "center"
    :display "inline-block" 
    :width "56px"
    :font-size "9px"
    ...
    ::stylefy/mode {
      :hover {
        :background-color (htstyle/colors :alumina-grey)
        :color (htstyle/colors :white)
      }
    }}))
...
(defn add-days [{:keys [day month year]} {:keys [days months years]}]
  (let [jsDate (js/Date. year (dec month) day)]        
    ...
    {:day (.getDate jsDate) :month (inc (.getMonth jsDate)) :year (.getFullYear jsDate)}
    )
  )

(defn- dates-to-show [year month week-start]
  (let [
    full-date (fn [d m y] {:day d :month m :year y})
    ...
    days-pmonth-actual (if (< days-pmonth-raw 0) (+ 7 days-pmonth-raw) days-pmonth-raw)
    days-to-show-pc (into [] (concat 
                (map full-date (range (->> days-pmonth-actual (- pmonth) (+ 1)) (+ pmonth 1)) 
                                (cycle [pmonthno]) (cycle [(if (= pmonthno 12) (- year 1) year)]))
                (map full-date (range 1 (+ 1 cmonth)) (cycle [month]) (cycle [year]))))
    all-days (->> (map full-date (->> days-to-show-pc (count) (- 43) (range 1)) 
                                    (cycle [(if (= month 12) 1 (+ 1 month))]) 
                                    (cycle [(if (= month 12) (+ 1 year) year)])) 
                  (concat days-to-show-pc) (into []))]
      all-days
    )
  )
...
(defn- dates-selector 
  [{:keys [year month week-start range on-date-click on-date-mouse-enter on-date-mouse-leave]}]
    [:div
      [:div
        [:div
          (doall (map (fn [%] ^{:key %} [:div (stylefy/use-style s-week-header)
                                                  (first %)]) 
                  (->> (week-days) (cycle) (take (+ 7 week-start)) (drop week-start))))]]
      [:div
      (let [weeks (partition 7 (dates-to-show year month week-start))]
        (doall (map 
          (fn [week] ^{:key week} [:div (doall (map (fn [day] 
                                   ^{:key day} 
                                   [:div (merge (stylefy/use-style s-date) 
                                        ...
                                        [:div (day :day)]
                                        [:div (stylefy/use-style (s-date-dots 
                                          (cond 
                                              (in-range? day (select-keys range [:start :end])) (htstyle/colors :sky-blue)
                                              (in-range? day (clset/rename-keys (select-keys range [:start :tempend]) {:tempend :end})) (htstyle/colors :sky-blue)
                                              (or (= (range :start) day) 
...
(defn- months-component [on-month-change]
  [:div
    (map (fn [m] ^{:key m}
      [:div.month { :style {:display "inline-block" :width "70px" :text-align "center" :padding "15px" :color (htstyle/colors :bitumen-grey)}
                    :on-click #(on-month-change m)} 
        (subs m 0 3)]) (month-names))
  ]
)

(defn- full-toggle-button [text is-selected? onclick]
  [:div (merge 
    (stylefy/use-style (s-full-toggle-button is-selected?))
...
(defn calendar-component [{:keys [selection-complete-event selection]}]
  ...
  (let [
    state (r/atom 
    {
      ...
      :month-to-show {:month (inc (.getMonth (js/Date.))) :year (.getFullYear (js/Date.))}
      :range {
        :start nil
        :end nil
        :tempend nil
    ...
    raise-selection-complete-event (fn []
      (let [
          start (get-in @state [:range :start])
    ...
    on-date-click (fn [day]
      (let [selected-range (:selected-range @state)
            to-add (some #(when (= (:name %) selected-range) (:to-add %)) range-options)]
        (if (= selection "range")
          (if (not (get-in @state [:range :start]))
            (do (swap! state assoc-in [:range :start] day)
              (if to-add (do 
                  (swap! state assoc-in [:range :end] (add-days day to-add))
    ...
    on-date-mouse-enter (fn [day]
      (if (and (= (:selected-range @state) "Custom") (= selection "range")) 
          (if (and (get-in @state [:range :start]) (not (get-in @state [:range :end])))
    ...
    on-month-change (fn [month]
      (let [month-no (->> (month-names) (keep-indexed (fn [i m] (if (= m month) i))) (first))]
    ...
    on-range-option-click (fn [button]
      ...
      (let [to-add (some #(when (= (:name %) (:name button)) (:to-add %)) range-options)
    ...
    (r/create-class {
      :reagent-render (fn []
      (let [        
        selected-month (get-in @state [:month-to-show :month])
        selected-year (get-in @state [:month-to-show :year])
        selected-range (@state :selected-range)
      ]
        [:div {:style {:width "235px" :font-size "11px" :user-select "none" :display "inline-block"}}
                [:div {:style {:padding "10px"}}
                
                [:div {:style {:text-align "center"  :cursor "pointer" :margin-bottom "15px"}}
                  [:div {:style {:display "inline-block" :width "15px" } :on-click (fn [] 
                        (swap! state assoc-in [:hide-content?] true)                
                        (js/setTimeout #(if (= "date" (get-in @state [:view])) 
                          (change-month dec) 
                          (change-year dec)) 100)
                        (js/setTimeout #(swap! state assoc-in [:hide-content?] false) 100)
                          )} "<"]
                  [:div { 
                      :on-click #(swap! state assoc-in [:view] "month")
                    :style { :display "inline-block" :text-align "center" :width "100px" :padding "2px" :font-weight "bold"}}
                ...
                [:div {:class (str "s-opacity-transition" (if (:hide-content? @state) " s-transparent" nil))}
                (if (= "date" (:view @state)) 
                  [:div 
                      [dates-selector {
                        :year selected-year
                ...
                (if (and (= "date" (:view @state)) (= selection "range")) 
                        [:div (stylefy/use-style s-range-options) 
                          (map (fn [op] ^{:key (:name op)}
                              [full-toggle-button (:name op) (= selected-range (:name op)) #(on-range-option-click op)]
                            ) range-options)
                        ]
                      )] 
      )
